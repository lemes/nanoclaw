...
The location server is part of the fork-local **sidecar** (`src/sidecar.ts`, the `com.nanoclaw.sidecar` service), not the main orchestrator. See `src/sidecar.ts` for the boundary.
...
Tailscale tunnel (encrypted WireGuard mesh)
  │
  ▼
NanoClaw location server (sidecar, port 7100)
  │  Parses OwnTracks payload, writes per-user file
  ▼
~/.local/share/nanoclaw/locations/:user.json
  │  Mounted read-only into agent containers at /workspace/locations/
  ▼
Agent reads on demand (via the `location-awareness` skill) when relevant
```

> **Storage moved (v2):** locations now live under `~/.local/share/nanoclaw/locations/`, **not** `groups/global/locations/`. The v2 CLAUDE.md cutover (`migrateGroupsToClaudeLocal` in `src/claude-md-compose.ts`) deletes `groups/global/` on every host boot, which would wipe locations stored there. Keeping the store under XDG data home (alongside the web-publish `apps/` dir) decouples it from that wipe and from group reshuffles.
...
A plain Node.js HTTP server started by the sidecar (`src/sidecar.ts`), not the main host process.
...
5. Writes to `~/.local/share/nanoclaw/locations/<user>.json`
...
The host mounts the location store read-only into **every** group at a single path (`src/container-runner.ts`):

| Mount path | Access |
|---|---|
| `/workspace/locations/*.json` | Read-only (via `~/.local/share/nanoclaw/locations` mount) |

There is no longer a separate "main group" path — v2 dropped the project-root mount, so every agent reads the same `/workspace/locations/` regardless of group.

### 5. Agent usage

The **`location-awareness` container skill** (`container/skills/location-awareness/SKILL.md`) teaches the agent to read `/workspace/locations/<user>.json` when location context is relevant — weather, nearby places, travel time, distance between users, etc. It's mounted into every session that uses `skills: all`. Each user has their own file (e.g. `vin.json`, `yasmin.json`). The agent reads on demand, not by polling, and checks the `timestamp` for freshness.

> This skill replaces the old `groups/global/CLAUDE.md` instruction, which the v2 cutover deleted — that deletion is what made the agent "forget" location after migration.
...
```bash
cat ~/.local/share/nanoclaw/locations/vin.json
```
...
Their location appears at `~/.local/share/nanoclaw/locations/<their-name>.json` and is readable by all groups. No server-side configuration needed — just a new URL.
...
```
~/.local/share/nanoclaw/locations/   →  mounted RO at /workspace/locations/
├── vin.json
├── yasmin.json
└── ...
```
