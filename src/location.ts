...
import { LOCATIONS_DIR } from './config.js';
...
export function startLocationServer(): http.Server {
    ...
    if (req.method === 'POST' && user) {
      ...
      req.on('end', () => {
        try {
          ...
          log.warn('Invalid location payload', { err, user });
  ...
  // Ensure the store exists on boot so container mounts (/workspace/locations)
  // resolve even before the first OwnTracks POST arrives.
  fs.mkdirSync(LOCATIONS_DIR, { recursive: true });

  server.listen(LOCATION_PORT, () => {
    log.info('Location server listening', { port: LOCATION_PORT });
