  ...
  async connect(): Promise<void> {
    ...
    this.bot.on('message:text', async (ctx) => {
      ...
      if (botUsername) {
        ...
        // Also handle plain-text @bot_username (no entity) — e.g. typed manually
        if (
          !TRIGGER_PATTERN.test(content) &&
          content.toLowerCase().startsWith(`@${botUsername}`)
        ) {
          content = `@${ASSISTANT_NAME} ${content}`;
        }
      }
    ...
    this.bot.on('message:photo', async (ctx) => {
      ...
      try {
        ...
        logger.warn(
          { err, chatJid },
          'Telegram image - download/process failed',
        );
      }
    ...
    this.bot.on('message:voice', async (ctx) => {
      ...
      try {
        ...
        logger.warn({ err }, 'Failed to download/transcribe voice message');
      }
