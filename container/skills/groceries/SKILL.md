...
**Database:** `/workspace/agent/groceries/groceries.db` (read-only queries).

The container image has no `sqlite3` CLI — query with **`bun:sqlite`** instead. First check
the DB exists (it lives in the main chat's workspace); if not, the data isn't available here:

```bash
test -f /workspace/agent/groceries/groceries.db || echo "Groceries data isn't available in this chat."
```

**Query pattern** — write the SQL into a small bun script and run it (avoids shell-quoting
issues with SQLite string literals):

```bash
cat > /tmp/gq.ts << 'EOF'
import { Database } from "bun:sqlite";
const db = new Database("/workspace/agent/groceries/groceries.db", { readonly: true });
const sql = `
  SELECT normalized_name, COUNT(DISTINCT receipt_id) AS trips
...
Replace the `sql` template with any query from the reference below, then run it.
