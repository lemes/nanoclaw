...
Wire-level control of an NVIDIA Shield TV via ADB over WiFi. The host Mac's ADB keypair is mounted read-only at `/workspace/extra/android` and adb is pointed at it via the `ADB_VENDOR_KEYS` env var, so the Shield accepts the container's identity without a TV-side authorization dialog (the Shield already trusts this host's key).
...
```bash
adb -s "$SHIELD_IP" push /workspace/agent/somefile /sdcard/Download/
adb -s "$SHIELD_IP" pull /sdcard/Download/somefile /workspace/agent/
```
...
- If `adb connect` returns **"failed to authenticate"**, the RSA key mount isn't in place; check that `/workspace/extra/android/adbkey` exists and `ADB_VENDOR_KEYS` points at it (host `~/.android` must exist + be allowlisted and mounted into the container).
