...
A cooking assistant web app. Recipes live in the shared publishing space:
- **Engine**: `/workspace/extra/apps/cook-mode.html` (generic, loads any recipe)
- **Index**: `/workspace/extra/apps/recipes.html` (lists all recipes)
- **Recipe files**: `/workspace/extra/apps/recipes/*.json`
- **Base URL**: `https://viniciuss-macbook-pro.tailc7cd9d.ts.net:8443/apps/` (served by Caddy from the shared `/apps/` space; only publisher groups can write it)

To add a new recipe:
1. Create `/workspace/extra/apps/recipes/<id>.json` following the schema below
...
A saved browser session is at `/workspace/agent/pickuplimes-browser-state.json`. Load it to access premium recipes.

```bash
agent-browser state load /workspace/agent/pickuplimes-browser-state.json
...
8. Save new state: `agent-browser state save /workspace/agent/pickuplimes-browser-state.json`
